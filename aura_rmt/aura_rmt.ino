 ...
 *
 * Использование:
 *   - Живой режим (по умолчанию): декодирует LED0 каждый кадр и управляет двумя PWM LED
 *     по цвету: красный канал (R) -> внешний красный LED на GPIO13, синий канал
 *     (B) -> встроенный синий LED на GPIO2. Яркость каждого индикатора (PWM 0..255)
 *     следует за интенсивностью канала, отправленной контроллером ARGB.
